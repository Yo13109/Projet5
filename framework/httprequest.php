...
class HttpRequest
{
	...
	{
		$url = str_replace($basepath,"",$this->getUrl());
		if (preg_match("#^" . $this->_route->getPath() . "\/?$#", $url, $listRouteParam) > 0) {
			for ($i = 0; $i <= count($listRouteParam) - 2; $i++) {

				$this->_param[] = $listRouteParam[$i + 1];
			}
		}
	}
	...
	{
		$this->bindParam();
		$this->bindRouteParam($config->basepath);
